...
void OBJObjectTexture::draw(DrawData& data)
{
	//material.apply();
...
void OBJObjectTexture::centerObject()
{
	...
	while (itr != vertices->end())
	{
		...
		minX = std::min(vec[0], minX);
		maxX = std::max(vec[0], maxX);
		maxY = std::max(vec[1], maxY);
		minY = std::min(vec[1], minY);
		maxZ = std::max(vec[2], maxZ);
		minZ = std::min(vec[2], minZ);
	}
...
void OBJObjectTexture::scaleObject(float factor)
{
	...
	Matrix4 scale;
	scale.makeScale(factor * tanf(30.0f * M_PI / 180.0f) / length);
